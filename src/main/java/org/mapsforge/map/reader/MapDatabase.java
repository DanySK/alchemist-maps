 ...
 * 
 * @see <a
 *      href="https://code.google.com/p/mapsforge/wiki/SpecificationBinaryMapFile">Specification</a>
 */
public class MapDatabase {
    ...
    private void decodeWayNodesDoubleDelta(final LatLong[] waySegment) {
        ...
        for (int wayNodesIndex = 1; wayNodesIndex < waySegment.length; ++wayNodesIndex) {
            ...
            final double doubleDeltaLatitude = CoordinatesUtil.microdegreesToDegrees(this.readBuffer.readSignedInt());

            // get the way node longitude double-delta offset (VBE-S)
            final double doubleDeltaLongitude = CoordinatesUtil.microdegreesToDegrees(this.readBuffer.readSignedInt());

            final double singleDeltaLatitude = doubleDeltaLatitude + previousSingleDeltaLatitude;
            final double singleDeltaLongitude = doubleDeltaLongitude + previousSingleDeltaLongitude;
    ...
    private void decodeWayNodesSingleDelta(final LatLong[] waySegment) {
        ...
        for (int wayNodesIndex = 1; wayNodesIndex < waySegment.length; ++wayNodesIndex) {
            ...
            wayNodeLongitude = wayNodeLongitude + CoordinatesUtil.microdegreesToDegrees(this.readBuffer.readSignedInt());
     ...
     */
    public FileOpenResult openFile(final File mapFile) {
        try {
            if (mapFile == null) {
            ...
            final FileOpenResult fileOpenResult = this.mapFileHeader.readHeader(this.readBuffer, this.fileSize);
    ...
    private PoiWayBundle processBlock(final QueryParameters queryParameters, final SubFileParameter subFileParameter) {
        if (!processBlockSignature()) {
            return null;
        }

        final int[][] zoomTable = readZoomTable(subFileParameter);
        if (zoomTable == null) {
            return null;
        }
        final int zoomTableRow = queryParameters.getQueryZoomLevel() - subFileParameter.getZoomLevelMin();
        final int poisOnQueryZoomLevel = zoomTable[zoomTableRow][0];
        final int waysOnQueryZoomLevel = zoomTable[zoomTableRow][1];
        ...
        final List<PointOfInterest> pois = processPOIs(poisOnQueryZoomLevel);
        if (pois == null) {
            return null;
        }
        ...
        final List<Way> ways = processWays(queryParameters, waysOnQueryZoomLevel);
        if (ways == null) {
            return null;
        }
    ...
    private MapReadResult processBlocks(final QueryParameters queryParameters, final SubFileParameter subFileParameter) throws IOException {
        boolean queryIsWater = true;
        boolean queryReadWaterInfo = false;

        final MapReadResultBuilder mapReadResultBuilder = new MapReadResultBuilder();

        // read and process all blocks from top to bottom and from left to right
        for (long row = queryParameters.getFromBlockY(); row <= queryParameters.getToBaseTileY(); ++row) {
            for (long column = queryParameters.getFromBlockX(); column <= queryParameters.getToBlockX(); ++column) {
                // calculate the actual block number of the needed block in the
                // file
                final long blockNumber = row * subFileParameter.getBlocksWidth() + column;

                // get the current index entry
                final long currentBlockIndexEntry = this.databaseIndexCache.getIndexEntry(subFileParameter, blockNumber);
                ...
                final long currentBlockPointer = currentBlockIndexEntry & BITMASK_INDEX_OFFSET;
                if (currentBlockPointer < 1 || currentBlockPointer > subFileParameter.getSubFileSize()) {
                    LOGGER.warning("invalid current block pointer: " + currentBlockPointer);
                    LOGGER.warning("subFileSize: " + subFileParameter.getSubFileSize());
                    return null;
                }

                long nextBlockPointer;
                // check if the current block is the last block in the file
                if (blockNumber + 1 == subFileParameter.getNumberOfBlocks()) {
                    // set the next block pointer to the end of the file
                    nextBlockPointer = subFileParameter.getSubFileSize();
                } else {
                    // get and check the next block pointer
                    nextBlockPointer = this.databaseIndexCache.getIndexEntry(subFileParameter, blockNumber + 1) & BITMASK_INDEX_OFFSET;
                    if (nextBlockPointer > subFileParameter.getSubFileSize()) {
                        LOGGER.warning("invalid next block pointer: " + nextBlockPointer);
                        LOGGER.warning("sub-file size: " + subFileParameter.getSubFileSize());
                        return null;
                    }
                }

                // calculate the size of the current block
                final int currentBlockSize = (int) (nextBlockPointer - currentBlockPointer);
                ...
                this.inputFile.seek(subFileParameter.getStartAddress() + currentBlockPointer);
                ...
                this.tileLatitude = MercatorProjection.tileYToLatitude(subFileParameter.getBoundaryTileTop() + row, subFileParameter.getBaseZoomLevel());
                this.tileLongitude = MercatorProjection.tileXToLongitude(subFileParameter.getBoundaryTileLeft() + column, subFileParameter.getBaseZoomLevel());

                try {
                    final PoiWayBundle poiWayBundle = processBlock(queryParameters, subFileParameter);
    ...
    private List<PointOfInterest> processPOIs(final int numberOfPois) {
        final List<PointOfInterest> pois = new ArrayList<PointOfInterest>();
        final Tag[] poiTags = this.mapFileHeader.getMapFileInfo().getPoiTags();
        ...
        for (int elementCounter = numberOfPois; elementCounter != 0; --elementCounter) {
            ...
            final double latitude = this.tileLatitude + CoordinatesUtil.microdegreesToDegrees(this.readBuffer.readSignedInt());

            // get the POI longitude offset (VBE-S)
            final double longitude = this.tileLongitude + CoordinatesUtil.microdegreesToDegrees(this.readBuffer.readSignedInt());

            // get the special byte which encodes multiple flags
            final byte specialByte = this.readBuffer.readByte();

            // bit 1-4 represent the layer
            final byte layer = (byte) ((specialByte & POI_LAYER_BITMASK) >>> POI_LAYER_SHIFT);
            // bit 5-8 represent the number of tag IDs
            final byte numberOfTags = (byte) (specialByte & POI_NUMBER_OF_TAGS_BITMASK);

            final List<Tag> tags = new ArrayList<Tag>();

            // get the tag IDs (VBE-U)
            for (byte tagIndex = numberOfTags; tagIndex != 0; --tagIndex) {
                final int tagId = this.readBuffer.readUnsignedInt();
            ...
            final byte featureByte = this.readBuffer.readByte();

            // bit 1-3 enable optional features
            final boolean featureName = (featureByte & POI_FEATURE_NAME) != 0;
            final boolean featureHouseNumber = (featureByte & POI_FEATURE_HOUSE_NUMBER) != 0;
            final boolean featureElevation = (featureByte & POI_FEATURE_ELEVATION) != 0;
    ...
    private LatLong[][] processWayDataBlock(final boolean doubleDeltaEncoding) {
        // get and check the number of way coordinate blocks (VBE-U)
        final int numberOfWayCoordinateBlocks = this.readBuffer.readUnsignedInt();
        ...
        final LatLong[][] wayCoordinates = new LatLong[numberOfWayCoordinateBlocks][];
        ...
        for (int coordinateBlock = 0; coordinateBlock < numberOfWayCoordinateBlocks; ++coordinateBlock) {
            ...
            final int numberOfWayNodes = this.readBuffer.readUnsignedInt();
            ...
            final LatLong[] waySegment = new LatLong[numberOfWayNodes];
    ...
    private List<Way> processWays(final QueryParameters queryParameters, final int numberOfWays) {
        final List<Way> ways = new ArrayList<Way>();
        final Tag[] wayTags = this.mapFileHeader.getMapFileInfo().getWayTags();
        ...
        for (int elementCounter = numberOfWays; elementCounter != 0; --elementCounter) {
            ...
            final int wayDataSize = this.readBuffer.readUnsignedInt();
            if (wayDataSize < 0) {
            ...
            if (queryParameters.isUseTileBitmask()) {
                // get the way tile bitmask (2 bytes)
                final int tileBitmask = this.readBuffer.readShort();
                // check if the way is inside the requested tile
                if ((queryParameters.getQueryTileBitmask() & tileBitmask) == 0) {
            ...
            // bit 1-4 represent the layer
            final byte layer = (byte) ((specialByte & WAY_LAYER_BITMASK) >>> WAY_LAYER_SHIFT);
            // bit 5-8 represent the number of tag IDs
            final byte numberOfTags = (byte) (specialByte & WAY_NUMBER_OF_TAGS_BITMASK);
            ...
            final boolean featureName = (featureByte & WAY_FEATURE_NAME) != 0;
            final boolean featureHouseNumber = (featureByte & WAY_FEATURE_HOUSE_NUMBER) != 0;
            final boolean featureRef = (featureByte & WAY_FEATURE_REF) != 0;
            final boolean featureLabelPosition = (featureByte & WAY_FEATURE_LABEL_POSITION) != 0;
            final boolean featureWayDataBlocksByte = (featureByte & WAY_FEATURE_DATA_BLOCKS_BYTE) != 0;
            final boolean featureWayDoubleDeltaEncoding = (featureByte & WAY_FEATURE_DOUBLE_DELTA_ENCODING) != 0;
            ...
            final LatLong labelPosition = readOptionalLabelPosition(featureLabelPosition);

            final int wayDataBlocks = readOptionalWayDataBlocksByte(featureWayDataBlocksByte);
            if (wayDataBlocks < 1) {
            ...
            for (int wayDataBlock = 0; wayDataBlock < wayDataBlocks; ++wayDataBlock) {
                final LatLong[][] wayNodes = processWayDataBlock(featureWayDoubleDeltaEncoding);
                if (wayNodes == null) {
                    return null;
                }
    ...
    /**
     * Reads all map data for the area covered by the given tile at the tile
     * zoom level.
     * 
     * @param tile
    ...
    public MapReadResult readMapData(final Tile tile) {
        try {
            ...
            final SubFileParameter subFileParameter = this.mapFileHeader.getSubFileParameter(queryParameters.getQueryZoomLevel());
    ...
    private LatLong readOptionalLabelPosition(final boolean featureLabelPosition) {
    ...
    private int readOptionalWayDataBlocksByte(final boolean featureWayDataBlocksByte) {
    ...
    private int[][] readZoomTable(final SubFileParameter subFileParameter) {
        final int rows = subFileParameter.getZoomLevelMax() - subFileParameter.getZoomLevelMin() + 1;
        final int[][] zoomTable = new int[rows][2];
